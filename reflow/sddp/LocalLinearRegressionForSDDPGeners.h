...
#include <memory>
#include "reflow/sddp/LocalLinearRegressionForSDDP.h"
...
///@{
gs_specialize_class_id(reflow::LocalLinearRegressionForSDDP, 1)
/// an external class
gs_declare_type_external(reflow::LocalLinearRegressionForSDDP)
///@}
...
struct GenericWriter < Stream, State, reflow::LocalLinearRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool process(const reflow::LocalLinearRegressionForSDDP &p_regression, Stream &p_os,
                               State *, const bool p_processClassId)
    {
        // If necessary, serialize the class id
        static const ClassId current(ClassId::makeId<reflow::LocalLinearRegressionForSDDP>());
...
//
template <class Stream, class State>
struct GenericReader < Stream, State, reflow::LocalLinearRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool readIntoPtr(reflow::LocalLinearRegressionForSDDP  *&ptr, Stream &p_is,
                                   State *p_st, const bool p_processClassId)
    {
        ...
        if (ptr)
        {
            *ptr = reflow::LocalLinearRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg, *diagReg, *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        }
        else
        {
            ptr = new  reflow::LocalLinearRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg, *diagReg, *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        }

        return true;
    }

    inline static bool process(reflow::LocalLinearRegressionForSDDP &s, Stream &is,
                               State *st, const bool p_processClassId)
    {
        // Simply convert reading by reference into reading by pointer
        reflow::LocalLinearRegressionForSDDP *ps = &s;
