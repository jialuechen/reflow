...
///@{
gs_specialize_class_id(reflow::SDDPVisitedStates, 1)
gs_declare_type_external(reflow::SDDPVisitedStates)
///@}
namespace gs
{
//
...
struct GenericWriter < Stream, State, reflow::SDDPVisitedStates,   Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool process(const reflow::SDDPVisitedStates &p_state, Stream &p_os,
                               State *, const bool p_processClassId)
    {
        // If necessary, serialize the class id
        static const ClassId current(ClassId::makeId<reflow::SDDPVisitedStates>());
...
//
template <class Stream, class State>
struct GenericReader < Stream, State, reflow::SDDPVisitedStates,   Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool readIntoPtr(reflow::SDDPVisitedStates *&ptr, Stream &p_is,
                                   State *p_st, const bool p_processClassId)
    {
        ...
        if (ptr)
            *ptr = reflow::SDDPVisitedStates(*meshToState, *states, *mesh);
        else
            ptr = new  reflow::SDDPVisitedStates(*meshToState, *states, *mesh);
        return true;
    }

    inline static bool process(reflow::SDDPVisitedStates &s, Stream &is,
                               State *st, const bool p_processClassId)
    {
        // Simply convert reading by reference into reading by pointer
        reflow::SDDPVisitedStates *ps = &s;
