...
public :
    ...
    virtual Eigen::ArrayXd oneStepBackward(const reflow::SDDPCutOptBase &p_linCut, const std::tuple< std::shared_ptr<Eigen::ArrayXd>, int, int > &p_aState, const Eigen::ArrayXd &p_particle, const int &p_isample) const = 0;
    ...
    virtual double oneStepForward(const Eigen::ArrayXd &p_aParticle, Eigen::ArrayXd &p_state,  Eigen::ArrayXd &p_stateToStore, const reflow::SDDPCutOptBase &p_linCut,
    ...
    virtual std::shared_ptr< reflow::SimulatorSDDPBase > getSimulatorBackward() const = 0;

    /// \brief get the forward simulator back
    virtual std::shared_ptr< reflow::SimulatorSDDPBase > getSimulatorForward() const = 0;
