...
private :
    ...
    std::shared_ptr<reflow::OptimizerDPCutBase >          m_pOptimize ; ///< optimizer solving the problem for one point and one step
...
public :
    ...
    SimulateStepRegressionCut(gs::BinaryFileArchive &p_ar,  const int &p_iStep,  const std::string &p_nameCont,
                              const   std::shared_ptr<SpaceGrid> &p_pGridFollowing, const  std::shared_ptr<reflow::OptimizerDPCutBase > &p_pOptimize
#ifdef USE_MPI
