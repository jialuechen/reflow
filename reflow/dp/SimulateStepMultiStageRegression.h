...
private :
    ...
    std::shared_ptr<reflow::OptimizerMultiStageDPBase >          m_pOptimize ; ///< optimizer solving the problem for one point and one step
...
public :
    ...
    SimulateStepMultiStageRegression(std::shared_ptr<gs::BinaryFileArchive>  &p_ar,  const int &p_iStep,  const std::string &p_nameCont, const std::string &p_nameDetCont,
                                     const   std::shared_ptr<SpaceGrid> &p_pGridFollowing, const  std::shared_ptr<reflow::OptimizerMultiStageDPBase > &p_pOptimize
#ifdef USE_MPI
