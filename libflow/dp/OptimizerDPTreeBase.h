...
public :
    ...
    virtual std::pair< Eigen::ArrayXXd, Eigen::ArrayXXd>   stepOptimize(const   std::shared_ptr< libflow::SpaceGrid> &p_grid, const Eigen::ArrayXd   &p_stock,
            const std::vector< libflow::ContinuationValueTree  > &p_condEsp) const = 0;
    ...
    virtual void stepSimulate(const std::shared_ptr< libflow::SpaceGrid>   &p_grid, const std::vector< libflow::GridTreeValue  > &p_continuation,
    ...
    virtual std::shared_ptr< libflow::SimulatorDPBaseTree > getSimulator() const = 0;
