...
SimulateStepTreeControl::SimulateStepTreeControl(gs::BinaryFileArchive &p_ar,  const int &p_iStep,  const string &p_nameCont,
        const   shared_ptr<SpaceGrid> &p_pGridFollowing,
        const  shared_ptr<libflow::OptimizerDPTreeBase > &p_pOptimize
#ifdef USE_MPI
...
#endif
{
    gs::Reference< vector<GridTreeValue> > (p_ar, (p_nameCont + "Control").c_str(), boost::lexical_cast<string>(p_iStep).c_str()).restore(0, &m_control);
}

void SimulateStepTreeControl::oneStep(vector<StateTreeStocks > &p_statevector, ArrayXXd  &p_phiInOut) const
{
#ifdef USE_MPI
