...
SimulateStepRegressionDist::SimulateStepRegressionDist(gs::BinaryFileArchive &p_ar,  const int &p_iStep,  const string &p_nameCont,
    ...
    m_pOptimize(p_pOptimize), m_contValue(p_pOptimize->getNbRegime()), m_bOneFile(p_bOneFile)	, m_world(p_world)
{
    ...
    if (m_bOneFile)
    {
        gs::Reference< vector< GridAndRegressedValue> >(p_ar, (p_nameCont + "Values").c_str(), stepString.c_str()).restore(0, &m_continuationObj);
    }
    else
    {
...
void SimulateStepRegressionDist::oneStep(vector<StateWithStocks > &p_statevector, ArrayXXd  &p_phiInOut) const
{
    ...
    else
    {
        ...
        vector< GridAndRegressedValue >  continuationExtended(m_pOptimize->getNbRegime());
        for (int iReg = 0; iReg < m_pOptimize->getNbRegime(); ++iReg)
        {
            ...
            continuationExtended[iReg] = GridAndRegressedValue(gridExtended, m_regressor);
            // affect
