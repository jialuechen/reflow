...
#include <memory>
#include "libflow/sddp/LocalConstRegressionForSDDP.h"
...
///@{
gs_specialize_class_id(libflow::LocalConstRegressionForSDDP, 1)
/// an external class
gs_declare_type_external(libflow::LocalConstRegressionForSDDP)
///@}
...
struct GenericWriter < Stream, State, libflow::LocalConstRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool process(const libflow::LocalConstRegressionForSDDP &p_regression, Stream &p_os,
                               State *, const bool p_processClassId)
    {
        // If necessary, serialize the class id
        static const ClassId current(ClassId::makeId<libflow::LocalConstRegressionForSDDP>());
...
//
template <class Stream, class State>
struct GenericReader < Stream, State, libflow::LocalConstRegressionForSDDP,
           Int2Type<IOTraits<int>::ISEXTERNAL> >
{
    inline static bool readIntoPtr(libflow::LocalConstRegressionForSDDP  *&ptr, Stream &p_is,
                                   State *p_st, const bool p_processClassId)
    {
        ...
        if (ptr)
            *ptr = libflow::LocalConstRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg, *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        else
        {
            ptr = new  libflow::LocalConstRegressionForSDDP(bZeroDate, *particles, *nbMesh, *mesh, *mesh1D, *simToCell, *matReg,  *simulBelongingToCell, *meanX, *etypX, *svdMatrix, bRotationAndRescale);
        }
        return true;
    }

    inline static bool process(libflow::LocalConstRegressionForSDDP &s, Stream &is,
                               State *st, const bool p_processClassId)
    {
        // Simply convert reading by reference into reading by pointer
        libflow::LocalConstRegressionForSDDP *ps = &s;
