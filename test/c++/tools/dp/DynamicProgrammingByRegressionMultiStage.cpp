...
double  DynamicProgrammingByRegressionMultiStage(const shared_ptr<reflow::FullGrid> &p_grid,
                                                ...
                                                )
{
    ...
    for (int iStep = 0; iStep < simulator->getNbStep(); ++iStep)
    {
        ...
        reflow::TransitionStepMultiStageRegressionDP transStep(p_grid, p_grid, p_optimize, ar, toStorBellDet
#ifdef USE_MPI
                , p_world
#endif
                                                             );
