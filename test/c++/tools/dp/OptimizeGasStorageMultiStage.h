...
class OptimizeGasStorageMultiStage : public reflow::OptimizerMultiStageDPBase
{
private :
...
public :
    ...
    Eigen::ArrayXXd stepOptimize(const   std::shared_ptr< reflow::SpaceGrid> &p_grid, const Eigen::ArrayXd   &p_stock,
                                 const std::vector<std::shared_ptr<reflow::ContinuationValue>> &p_condEsp,
                                 const std::vector < std::shared_ptr< Eigen::ArrayXXd > > &p_phiIn) const
    {
        ...
        if ((reflow::isStrictlyLesser(injectionMax, 0.) && reflow::isStrictlyLesser(withdrawalMax, 0.)) ||
                (reflow::isStrictlyLesser(withdrawalMax, -injectionMax)))
        {
            // not an admissible point
            solution.setConstant(-reflow::infty);
            return solution;
        }
    ...
    inline std::shared_ptr< reflow::SimulatorMultiStageDPBase > getSimulator() const
    {
        return m_simulator ;
    }
