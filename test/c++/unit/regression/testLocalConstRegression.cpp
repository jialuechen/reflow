...
#define BOOST_TEST_MODULE testLocalConstRegression
...
public :
    ...
    double operator()(const double &p_x)
    {
...
void testDimension(const bool &p_bRotate, const int &p_nDim, const int &p_nbSimul, const int &p_nMesh, const double &p_accuracyRegression)
{
    ...
    // check if regression in t=0
    {
        ...
        bool bZeroDate = true;
        // constructor
        LocalConstRegression localRegressor(nbMesh, p_bRotate);
...
BOOST_AUTO_TEST_CASE(testLocalRegression1DMultipleSecondMember)
{
    ...
    // check if regression in t=0
    {
        ...
        bool bZeroDate = true;
        // constructor
        LocalConstRegression localRegressor(nbMesh);
        localRegressor.updateSimulations(bZeroDate, x);
        // regression
    ...
    // t is not zero
    {
        ...
        bool bZeroDate = 0;
        // constructor
        LocalConstRegression localRegressor(bZeroDate, x, nbMesh);
        // regression
        ArrayXXd  regressedValues = localRegressor.getAllSimulationsMultiple(toRegress);
...
// test serialization LocalConstRegression
void  TestLocalRegressionSerialization(const int &p_nDim, const int &p_nbSimul, const int &p_nbMesh)
{
    ...
    // test archive
    {
        ...
        // archive
        BinaryFileArchive ar("archiveLCR", "w");
        ar << Record(*localRegressor, "Regressor", "Top") ;
        ar.flush();
    }
    {
        BinaryFileArchive ar("archiveLCR", "r");
        LocalConstRegression reg;
        Reference< LocalConstRegression> (ar, "Regressor", "Top").restore(0, &reg);
...
void TestContinuationValue(const int &p_nDim, const int &p_nbSimul, const int &p_nbMesh)
{
    ...
    ArrayXXd  regressedValues;
    {
        ...
        BinaryFileArchive ar("archiveLCR1", "w");
        ar << Record(continuation, "FirstContinuation", "Top") ;
        ar.flush();
    }
    {
        // read archive
        BinaryFileArchive ar("archiveLCR1", "r");
