...
void testDimension(const bool &p_bRotate, const int &p_nDim, const int &p_nbSimul, const int &p_nMesh, const double &p_accuracyRegression)
{
    ...
    // check if regression in t=0
    {
        ...
        bool bZeroDate = true;
        // constructor
        LocalLinearRegression localRegressor(nbMesh, p_bRotate);
...
BOOST_AUTO_TEST_CASE(testLocalRegression3D)
{
#if defined   __linux
    enable_abort_on_floating_point_exception();
#endif
...
BOOST_AUTO_TEST_CASE(testLocalRegression1DMultipleSecondMember)
{
    ...
    // t is not zero
    {
        ...
        bool bZeroDate = 0;
        // constructor
        LocalLinearRegression localRegressor(bZeroDate, x, nbMesh);
        // regression
        ArrayXXd  regressedValues = localRegressor.getAllSimulationsMultiple(toRegress);
...
// test serialization LocalLinearRegression
void  TestLocalRegressionSerialization(const int &p_nDim, const int &p_nbSimul, const int &p_nbMesh)
{
    ...
    // test archive
    {
        ...
        shared_ptr<LocalLinearRegression> localRegressor = make_shared<LocalLinearRegression>(false, x, nbMesh);
        ...
        // archive
        {
            BinaryFileArchive ar("archiveLLR", "w");
            ar << Record(*localRegressor, "Regressor", "Top") ;
        }
        {
            BinaryFileArchive ar("archiveLLR", "r");
            LocalLinearRegression reg;
            Reference< LocalLinearRegression> (ar, "Regressor", "Top").restore(0, &reg);
...
void TestContinuationValue(const int &p_nDim, const int &p_nbSimul, const int &p_nbMesh)
{
    ...
    ArrayXXd  regressedValues;
    {
        ...
        BinaryFileArchive ar("archiveLLR1", "w");
        ar << Record(continuation, "FirstContinuation", "Top") ;

    }
    {
        // read archive
        BinaryFileArchive ar("archiveLLR1", "r");
...
void TestContinuationCuts(const int &p_nDim, const int &p_nbSimul, const int &p_nbMesh)
{
...

    {
        ...
        // default non compression
        BinaryFileArchive ar("archiveLLR2", "w");
        ar << Record(contCut, "FirstContinuation", "Top") ;

    }
    {
        // read archive
        BinaryFileArchive ar("archiveLLR2", "r");
...
BOOST_AUTO_TEST_CASE(testContCut)
{
#if defined   __linux
    enable_abort_on_floating_point_exception();
#endif
