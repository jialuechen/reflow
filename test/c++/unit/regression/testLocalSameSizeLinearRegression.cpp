...
#define BOOST_TEST_MODULE testLocalSameSizeLinearRegression
...
#include "libflow/regression/LocalSameSizeLinearRegressionGeners.h"
...
void testDimension(const int &p_nDim, const int &p_nbSimul, const int &p_nMesh, const double &p_accuracyRegression)
{
    ...
    // check if regression in t=0
    {
        ...
        bool bZeroDate = true;
        // constructor
        LocalSameSizeLinearRegression localRegressor(lowValues, step, nbStep);
...
BOOST_AUTO_TEST_CASE(testLocalRegression1D)
{
    ...
    int nbSimul = 1000000;
    // accuracy
    double accuracy = 3; // percentage error
...
// 2D check single second member : only to test that if some cells are without simulations it doesn't crash
BOOST_AUTO_TEST_CASE(testLocalRegression2DNotALotOfSim)
{
#if defined   __linux
    enable_abort_on_floating_point_exception();
#endif
    ...
    int nbSimul = 400;
    // accuracy
    double accuracy = 1000; // percentage error : should be used
    testDimension(nDim, nbSimul, nMesh, accuracy);
}
...
BOOST_AUTO_TEST_CASE(testLocalRegression1DMultipleSecondMember)
{
    ...
    // t is not zero
    {
        ...
        bool bZeroDate = 0;
        // constructor
        LocalSameSizeLinearRegression localRegressor(bZeroDate, x, lowValues, step, nbStep);
        // regression
        ArrayXXd  regressedValues = localRegressor.getAllSimulationsMultiple(toRegress);
...
// test serialization LocalSameSizeLinearRegression
void  TestLocalRegressionSerialization(const int &p_nDim, const int &p_nbSimul, const int &p_nbMesh)
{
    ...
    // test archive
    {
        ...
        shared_ptr<LocalSameSizeLinearRegression> localRegressor = make_shared<LocalSameSizeLinearRegression>(false, x, lowValues, step, nbStep);
        ...
        // archive
        BinaryFileArchive ar("archiveLSSL", "w");
        ar << Record(*localRegressor, "Regressor", "Top") ;
    }
    {
        BinaryFileArchive ar("archiveLSSL", "r");
        LocalSameSizeLinearRegression reg;
        Reference< LocalSameSizeLinearRegression> (ar, "Regressor", "Top").restore(0, &reg);
...
void TestContinuationValue(const int &p_nDim, const int &p_nbSimul, const int &p_nbMesh)
{
    ...
    ArrayXXd  regressedValues;
    {
        ...
        // conditional expectation
        shared_ptr<LocalSameSizeLinearRegression> localRegressor = make_shared<LocalSameSizeLinearRegression>(false, x, lowValuesStoc, stepStoc, nbStepStoc);
        ...
        BinaryFileArchive ar("archiveLSSL1", "w");
        ar << Record(continuation, "FirstContinuation", "Top") ;

    }
    {
        // read archive
        BinaryFileArchive ar("archiveLSSL1", "r");
